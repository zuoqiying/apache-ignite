...
public class GridDeploymentMessageCountSelfTest extends GridCommonAbstractTest {
    ...
    public void testCacheValueDeploymentOnPut() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < 2; i++)
                assertNotNull("For grid: " + i, grid(i).cache(DEFAULT_CACHE_NAME).localPeek("key", CachePeekMode.OFFHEAP));
            ...
            for (int i = 0; i < 10; i++) {
                ...
                for (int k = 0; k < 2; k++)
                    assertNotNull(grid(k).cache(DEFAULT_CACHE_NAME).localPeek(key, CachePeekMode.OFFHEAP));
            }
