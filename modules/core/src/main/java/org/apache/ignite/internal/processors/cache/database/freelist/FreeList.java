...
public class FreeList {
    ...
    private final PageHandler<CacheDataRow, Integer> writeRow = new PageHandler<CacheDataRow, Integer>() {
        @Override public Integer run(long pageId, Page page, ByteBuffer buf, CacheDataRow row, int written)
            ...
            tree(cctx, row.partition()).put(new FreeItem(freeSpace, pageId));
     ...
     * @throws IgniteCheckedException If failed.
     */
    private FreeTree tree(GridCacheContext cctx, Integer partId) throws IgniteCheckedException {
     ...
     * @throws IgniteCheckedException If failed.
     */
    public void removeRow(GridCacheContext cctx, long link) throws IgniteCheckedException {
        assert link != 0;
        ...
        FreeTree tree = tree(cctx, partId);
    ...
    public void insertRow(CacheDataRow row) throws IgniteCheckedException {
        ...
        FreeTree tree = tree(cctx, row.partition());
        ...
        do {
            ...
            try (Page page = item == null ?
                allocateDataPage(cctx.cacheId(), row.partition()) :
     ...
     * @throws IgniteCheckedException If failed.
     */
    private Page allocateDataPage(int cacheId, int part) throws IgniteCheckedException {
